...
def extract_full_vector(frame_data):
    ...
    c_eyes = c_d_eyes  # 2 values
    c_lip = c_d_lip    # 1 value
...
def apply_encoder(vector, encoder):
    ...
    output_vector = encoder(torch.tensor(output_vector, dtype=torch.float32).cuda()).cpu().detach().numpy()

    return output_vector

# Apply weights to the components of the vector
def apply_encoder_exp_lip(vector, encoder):
    # Apply the weights to the respective parts
    weighted_vector = vector
