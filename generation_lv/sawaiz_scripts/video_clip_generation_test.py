...
def get_driving_dict(descriptors_list, fps=25):
    ...
    for i, des in enumerate(descriptors_list):
        ...
        c_eyes_lst = des["c_eyes_lst"][0]
        c_lip_lst = des["c_lip_lst"][0]
...
def map_image_using_folder(source, driving, output_dir, inference_cfg, crop_cfg, fps = 30):
     # for convenience
...
# cons block
    ...
    with open(f"video_all_dict.pkl", 'wb') as file:
        pickle.dump(driving_template_dct, file)
...
# 21.3
    ...
    if flag_is_driving_video or (flag_is_source_video and not flag_is_driving_video):
        ...
        if flag_source_has_audio or flag_driving_has_audio:
            ...
            wfp_concat_with_audio = osp.join(output_dir, f'{basename(source)}--{basename(driving)}_concat_with_audio.mp4')
        ...
        wfp = osp.join(args.output_dir, f'{basename(source)}--{basename(args.driving)}.mp4')
    ...
    if flag_source_has_audio or flag_driving_has_audio:
        print("22.2.")
        wfp_with_audio = osp.join(args.output_dir, f'{basename(source)}--{basename(driving)}_with_audio.mp4')
...
def map_image_using_descriptor_lst_test(source, template_dict, output_dir, output_vid_name, inference_cfg, crop_cfg, fps = 30):
     # for convenience
    inf_cfg = inference_cfg
    device = "cuda"
    crop_cfg = crop_cfg
 ...
 ### cons block
    ...
    driving_n_frames = len(template_dict["motion"])
...
#     # print(driving_template_dct)
    driving_template_dct, c_d_eyes_lst, c_d_lip_lst = template_dict, template_dict["c_eyes_lst"], template_dict["c_lip_lst"]
...
# map_image_using_folder("source_folder/2.jpg", "video_generation_test/01-01-02-02-02-02-02.mp4", "animations/", inference_cfg, crop_cfg)

output_dir = "animations/"
output_vid_name = "test"
import pickle
with open(f"video_all_dict.pkl", 'rb') as file:
    template_dct = pickle.load(file)

map_image_using_descriptor_lst_test("source_folder/2.jpg", template_dct, output_dir=output_dir, output_vid_name= output_vid_name, inference_cfg = inference_cfg, crop_cfg=crop_cfg, fps = 30)
